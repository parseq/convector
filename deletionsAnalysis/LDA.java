...
public class LDA implements Serializable{
    ...
    private double pairQChisq = 0.0; // quantile of chi squared distribution with 2 degree of freedom
    ...
    public LDA(Samples samples, HashMap<String, Amplicon> dataAboutAmplicons, ArrayList<String> namesOfSamples,
               ...
               int minNumOfModels, double qChisq, double pairQChisq) {
        this.qChisq = qChisq;
        this.pairQChisq = pairQChisq;
    ...
    public double getDist(double val, String amplName) {
        return (Math.pow(val - medians.get(amplName), 2) * diagMatrixOfCovariancesMap.get(amplName));
    }

    public double getDistDel(double val, String amplName) {
        return (Math.pow(val - medians.get(amplName) + logDel, 2) * diagMatrixOfCovariancesMap.get(amplName));
    }

    public double getDistDup(double val, String amplName) {
        return (Math.pow(val - medians.get(amplName) - logDup, 2) * diagMatrixOfCovariancesMap.get(amplName));
    }
    ...
    public void outRes(
        ...
        HashMap<String, HashMap<String, Integer>> candidateExonsAndSamples = new HashMap<String, HashMap<String, Integer>>();
        ...
        for (String name : samplesToCheck) {
            candidateExonsAndSamples.put(name, new HashMap<String, Integer>());
            ...
            for (Map.Entry<String, ArrayList<String>> exonAndListOfAmplicons : ampliconNamesFromExons.entrySet()) {
                candidateExonsAndSamples.get(name).put(exonAndListOfAmplicons.getKey(), 0);
            ...
            if (!nonEffectiveAmpls.contains(e.getKey()) && dataAboutAmplicons.containsKey(e.getKey())) {
                for (String name : samplesToCheck) {
                    ...
                    if ( // distance to Deletion model is not very big
                        (distToDel < qChisq) || distToDel < distToWt - qChisq) {
                        candidateExonsAndSamples.get(name).put(currentExon, candidateExonsAndSamples.get(name).get(currentExon) + 1);
                    } else if ( // distance to Duplication model is not very big
                        (distToDup < qChisq) || distToDup < distToWt - qChisq) {
                        candidateExonsAndSamples.get(name).put(currentExon, candidateExonsAndSamples.get(name).get(currentExon) - 1);
                    } else if (samplesToLDA.returnInitCov(name, e.getKey()) < 0.01) { // create a mark for determining Homozygous Deletion
        ...
        for (String name : samplesToCheck) {
            ...
            for (Map.Entry<String, ArrayList<String>> exonAndListOfAmplicons : ampliconNamesFromExons.entrySet()) {
                if (candidateExonsAndSamples.get(name).get(exonAndListOfAmplicons.getKey()) == exonAndListOfAmplicons.getValue().size()) {
                    deleteriousExons.get(name).add(exonAndListOfAmplicons.getKey());
                } else if ((candidateExonsAndSamples.get(name).get(exonAndListOfAmplicons.getKey()) == -exonAndListOfAmplicons.getValue().size())) {
                    duplicatedExons.get(name).add(exonAndListOfAmplicons.getKey());
                } else if ((candidateExonsAndSamplesHomoDel.get(name).get(exonAndListOfAmplicons.getKey()) == exonAndListOfAmplicons.getValue().size())) {
    ...
    private boolean notSimilarToNormalButSimilarToDeletion(String currentExon, double distExonToDel, double distExonToWt) {
         ...
         */
        return (
                // first case = an exon is covered with one amplicon
                (ampliconNamesFromExons.get(currentExon).size() == 1 && distExonToDel < distExonToWt && distExonToWt > qChisq)  ||
                        // second case = an exon is covered with two amplicons
                        (ampliconNamesFromExons.get(currentExon).size() == 2 && distExonToDel < distExonToWt && distExonToWt > pairQChisq) || (
                        // third case = an exon is covered with more than two amplicons
                        (ampliconNamesFromExons.get(currentExon).size() > 2 &&
                                distExonToDel < distExonToWt &&
                                distExonToWt > Statistics.getQuantile(ampliconNamesFromExons.get(currentExon).size(), 0.000001))
                )
        );
    }
    ...
    private boolean notSimilarToNormalButSimilarToDuplication(String currentExon, double distExonToDup, double distExonToWt) {
        ...
        return  (
                // first case = an exon is covered with one amplicon
                (ampliconNamesFromExons.get(currentExon).size() == 1 && distExonToDup < distExonToWt && distExonToWt > qChisq)  ||
                        // second case = an exon is covered with two amplicons
                        (ampliconNamesFromExons.get(currentExon).size() == 2 && distExonToWt > pairQChisq &&
                                distExonToDup < distExonToWt) ||
                        // third case = an exon is covered with more than two amplicons
                        (
                                (ampliconNamesFromExons.get(currentExon).size() > 2 && distExonToWt > Statistics.getQuantile(ampliconNamesFromExons.get(currentExon).size(), 0.01) &&
                                        distExonToDup < distExonToWt)
                        )
        );
    }
