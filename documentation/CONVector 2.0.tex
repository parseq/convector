...
\newcommand{\hmwkTitle}{CONVector\ 2.0 Package} % Assignment title
\newcommand{\hmwkDueDate}{December\ 19,\ 2015} % Due date
\newcommand{\hmwkClass}{Reference User Guide} % Course/class
\newcommand{\hmwkClassTime}{December\ 19,\ 2015} % Class/lecture time
...
\begin{usecase}
\addtitle{Use Case:}{Control dataset of more than 20 samples is available}
\addfield{Quick description}{You have a dataset clean of any CNVs and Test dataset.
}
\addscenario{Scenario Name:}{
\item Specify Control dataset with \texttt{-cs} flag for \texttt{pipeline.py} and also include the flag \texttt{-cc} in the command. \hyperlink{runWithStrictFiltration}{QC as high as suitable} (we would recommend 95\%) is required.
} 
\end{usecase}
...
\begin{center}
\begin{minipage}[h]{0.48\linewidth}
\includegraphics[width=1\linewidth]{del_example.png}
\caption{3 chromosomes: 7, 9, 12. Deletion in 7th chromosome.} 
\label{ris:del_ex} 
\end{minipage}
\hfill 
\begin{minipage}[h]{0.48\linewidth}
\includegraphics[width=1\linewidth]{dup_example.png}
...
Version 2.* - {\it Date of release.}
\begin{benumerate}{2}
\item {\it (June, 2015)} Major revision of second algorithm. Visualisation added.
\end{benumerate}
